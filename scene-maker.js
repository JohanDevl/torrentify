...
async function generatePrez(type, name, outDir, nfoPath, apiData, sourceFiles) {
...
`.trim());
  ...
  if (ENABLE_PREZ && !fs.existsSync(prezSerie)) {
    ...
    const cacheFile = path.join(CACHE_DIR, `${tmdbType}_${safeName(name).toLowerCase()}.json`);
    try { tmdbResult = JSON.parse(fs.readFileSync(cacheFile, 'utf8')); } catch {}
    if (tmdbResult) await generatePrez('serie', name, outDir, nfo, tmdbResult, videos);
  }
}
...
`.trim());
  ...
  if (ENABLE_PREZ && !fs.existsSync(prezMusic)) {
    ...
    if (itunesResult) await generatePrez('musique', name, outDir, nfo, itunesResult, allAudio);
  }
