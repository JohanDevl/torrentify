...
function generatePrez(type, name, outDir, nfoPath, apiData, sourceFiles) {
  if (!ENABLE_PREZ) return;
  const prezPath = path.join(outDir, `${name}.prez`);
  if (fs.existsSync(prezPath)) return;

  const nfoContent = extractMediaInfoFromNfo(nfoPath);
  const totalSize = getTotalSize(sourceFiles || []);
  const fileSize = totalSize > 0 ? formatSize(totalSize) : 'N/A';
  const fileCount = sourceFiles ? sourceFiles.length : 0;
  let bbcode = null;
  ...
  switch (type) {
    case 'film':
      ...
      break;
    case 'serie':
      bbcode = generateSeriePrez(name, nfoContent, apiData, fileSize, fileCount);
      break;
    case 'musique':
  ...
  if (bbcode) {
    ...
    prezGenerated++;
    console.log(`   📜 Prez générée : ${name}.prez`);
  }
}
...
async function processFile(file, destBase, index, total, label, tmdbType = 'movie', sourceDirs = []) {
  ...
  const prez = path.join(outDir, `${name}.prez`);
...
`.trim());
  ...
  if (ENABLE_PREZ && !fs.existsSync(prez)) {
    ...
    if (tmdbResult) generatePrez(tmdbType === 'tv' ? 'serie' : 'film', name, outDir, nfo, tmdbResult, [file]);
  }
...
`.trim());
  ...
  // 📜 Prez BBCode
  const prezSerie = path.join(outDir, `${name}.prez`);
  if (ENABLE_PREZ && !fs.existsSync(prezSerie)) {
    let tmdbResult = null;
    const cacheFile = path.join(CACHE_DIR, `${tmdbType}_${safeName(name).toLowerCase()}.json`);
    try { tmdbResult = JSON.parse(fs.readFileSync(cacheFile, 'utf8')); } catch {}
    if (tmdbResult) generatePrez('serie', name, outDir, nfo, tmdbResult, videos);
  }
}
...
async function processSeriesFolder(folder, destBase, index, total) {
  ...
  const srcInfo = path.join(outDir, `${name}.srcinfo`);
  const prezSeries = path.join(outDir, `${name}.prez`);
...
async function processMusicEntry(entryPath, destBase, index, total) {
  ...
  const prezMusic = path.join(outDir, `${name}.prez`);
  if (
    fs.existsSync(torrent) &&
    fs.existsSync(nfo) &&
    fs.existsSync(txt) &&
...
`.trim());
  ...
  if (ENABLE_PREZ && !fs.existsSync(prezMusic)) {
    ...
    if (itunesResult) generatePrez('musique', name, outDir, nfo, itunesResult, allAudio);
  }
